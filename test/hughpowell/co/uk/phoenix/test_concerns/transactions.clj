...
(defn generator [header-spec row-spec]
  (gen/fmap
    (fn [[header rows]] (->> rows sort (cons header)))
    (gen/tuple
      (spec/gen header-spec)
      (spec/gen row-spec))))


(defn write [input-path filename transactions]
  (with-open [writer (io/writer (fs/file input-path filename))]
